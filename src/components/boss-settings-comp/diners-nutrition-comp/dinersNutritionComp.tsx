...
class DinersNutritionComp extends PureComponent {
  ...
  constructor(props: {}) {
    ...
    this.disposeAutorun = autorun(() => {
      let meals: Meal[] = AppRootModelsContext.mealModel.objectList.map(
        (m) => m
      );
      if (this.isSetForAllMeals) {
        meals.forEach((m) =>
          m.diners.forEach((d) => (this.dinersNumber[d.dietType] = d.count))
        );
        this.totalDinersNum = meals[0]
          ? Object.values(this.dinersNumber).reduce((ttl, n) => ttl + n, 0)
          : 0;
      } else {
        ...
        if (this.selectedMealStore.meal) {
          this.selectedMealStore.meal.diners.forEach(
            (d) => (this.dinersNumber[d.dietType] = d.count)
          );
          this.totalDinersNum =
            this.selectedMealStore.meal.getTotalMealDiners();
        }
      }
      this.refreshData();
    });
  ...
  mealInfo(): string {
    ...
    if (this.selectedMealStore.meal)
      info = `${
        this.selectedMealStore.meal.chefName
      } - ${this.selectedMealStore.meal.date.toLocaleDateString("en-EN", {
        weekday: "long",
      })} ${this.selectedMealStore.meal.date.toLocaleDateString()}`;
    return info;
  }
  ...
  @action onChangeDinersNumber(
    e: React.ChangeEvent<any>,
    dietIndex: string
  ): void {
    this.dinersNumber[dietIndex] = Number(e.target.value);
    if (this.selectedMealStore.meal) {
      this.selectedMealStore.meal.updateDietTypeDinersNum(
        dietIndex,
        this.dinersNumber[dietIndex]
      );
    }
    if (this.isSetForAllMeals)
      AppRootModelsContext.mealModel.objectList.forEach((m) =>
        m.updateDietTypeDinersNum(dietIndex, this.dinersNumber[dietIndex])
      );
    this.refreshData();
  }
  ...
  @action onAddNewDietType(): void {
    if (this.selectedMealStore.meal)
      this.selectedMealStore.meal.addDietType(this.dietTypeName);
    if (this.isSetForAllMeals)
      AppRootModelsContext.mealModel.objectList.forEach((m) =>
        m.addDietType(this.dietTypeName)
      );
    this.refreshData();
    this.dietTypeName = "";
  }
  ...
  @action onDeleteDietType(dietName: string): void {
    if (this.selectedMealStore.meal)
      this.selectedMealStore.meal.deleteDietType(dietName);
    if (this.isSetForAllMeals)
      AppRootModelsContext.mealModel.objectList.forEach((m) =>
        m.deleteDietType(dietName)
      );
    this.refreshData();
  }
  ...
  @action onSwitch = (): void => {
    ...
    if (this.isSetForAllMeals) this.selectedMealStore.selectedDate = "";
    this.selectedMealStore.selectedMeal = "";
    this.selectedMealStore.meal = undefined;
    this.refreshData();
  };
  ...
  render() {
    ...
    dietTypes = Object.keys(this.dinersNumber).map((dietType) => (
      <div key={dietType} id="dietType">
        <Typography variant="h5" className="tgDiet">
          {dietType}:{" "}
        </Typography>
        <TextField
          label="Diners number"
          ...
          onChange={(e: React.ChangeEvent<any>) =>
            this.onChangeDinersNumber(e, dietType)
          }
          variant="outlined"
        ...
        <IconButton
          onClick={() => {
            this.onDeleteDietType(dietType);
          }}
        ...
        <Paper className="paper">
          <div className="switchForm">
            <FormControl component="fieldset">
              <RadioGroup
                aria-label="gender"
                name="gender1"
                value={this.isSetForAllMeals}
              >
                <FormControlLabel
                  value={true}
                  onClick={() => (this.isSetForAllMeals = true)}
                  control={
                    <Radio color="primary" checked={this.isSetForAllMeals} />
                  }
                  label={
                    <Box
                      className={`box ${this.isSetForAllMeals ? "on" : "off"}`}
                    >
                ...
                <FormControlLabel
                  ...
                  label={
                    <Box
                      className={`box ${this.isSetForAllMeals ? "off" : "on"}`}
                    >
          ...
          <div>
            <Typography variant="h4" className="tgTitle title">
              {this.isSetForAllMeals
                ? "Update All Exists Meals"
                : this.mealInfo()}
            </Typography>
            <TextField
              label="Enter new diet type"
              onChange={(e: React.ChangeEvent<any>) => {
                this.onEnterDietTypeName(e);
              }}
            ...
            <div
              id="dinersForm"
              className={`${
                this.isSetForAllMeals || this.selectedMealStore.meal
                  ? "visible"
                  : "hide"
              }`}
            >
              <div id="ttlDinersForm">
                <Typography variant="h5" id="tgTtlDiners">
                  Total Diners:{" "}
                </Typography>
                <TextField
                  label=""
              ...
              <div>
                <ExpansionPanel key={"info"} id="expnPnl">
                  ...
                  <p>
                    ★ Enter a new diet type, will add each meal the diet
                    category with its diners number.
                  </p>
                  <p>
                    ★ Delete diet type, will remove from each meal that has the
                    category with its diners number.
                  </p>
                  <p>
                    ★ If a new meal is added after setting all meals, it souled
                    be set manually select first the specific meal.
                  </p>
                  <p>
                    ★ Set select all meals, sets a diners number change for all
                    meals that already containing the type.
                  </p>
                  <p>
                    ★ Meals that hasn't the changed type won't change at all.
                  </p>
                  <p>
                    ★ If there is only one meal with unique type, it will
                    represent among set all meals option.{" "}
                  </p>
                </ExpansionPanel>
                <br />
              </div>
