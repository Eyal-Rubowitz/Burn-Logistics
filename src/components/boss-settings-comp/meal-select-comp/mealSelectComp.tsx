...
class MealSelectComp extends PureComponent<IMealSelectCompProps> {
  ...
  @computed get meals(): Meal[] {
    let ml = AppRootModelsContext.mealModel.objectList.map((m) => m);
    return ml;
  }
  ...
  @action onSelectMeal(ev: string): void {
    ...
    this.props.store.meal = AppRootModelsContext.mealModel.objectList.find(
      (m) =>
        m.date.toLocaleDateString() === this.selectedDate &&
        m.categoryType === this.selectedMeal
    );
  }

  render() {
    let chosenDateMeals: Meal[] = this.meals.filter(
      (m) => m.date.toLocaleDateString() === this.selectedDate
    );
    let dateEventListSet: Set<string> = new Set(
      chosenDateMeals.map((m) => m.categoryType)
    );
