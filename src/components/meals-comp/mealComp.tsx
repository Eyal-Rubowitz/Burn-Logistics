...
class MealComp extends PureComponent<IMealProps> {
  ...
  @observable servingTime: Date = new Date();
  ...
  constructor(props: IMealProps) {
    ...
    this.disposeAutorun = autorun(() => {
      ...
      if (this.meal) {
        mealIngs = Array.prototype.concat.apply(
          [],
          this.meal.dishes.map((d) => d.ingredients.map((ing) => ing))
        );
        this.mealQuantity = mealIngs.reduce((mealCost: number, ing) => {
          return (
            mealCost +
            (ing.getItemBaseUnit === "Kg" ? ing.convertedQuantity : 0)
          );
        }, 0);
        this.mealExpenses =
          AppRootModelsContext.ingredientModel.objectList.reduce(
            (mealCost: number, ing) => {
              return mealCost + ing.cost;
            },
            0
          );
  ...
  onAddDish(): void {
    if (this.meal) {
      ...
      let newDish: Dish = new Dish(AppRootModelsContext.dishModel, {
        _id: newId,
        name: "New Dish",
        mealId: this.meal._id,
      });
      AppRootModelsContext.dishModel.createObject(newDish);
    }
  }
  ...
  onUpdateTime(e: React.ChangeEvent<any>): void {
    ...
    if (this.meal) {
      focusId === "preparing"
        ? (this.preparationTime = new Date(
            this.meal.preparing.setHours(hour, min)
          ))
        : (this.servingTime = new Date(this.meal.serving.setHours(hour, min)));
      focusId === "preparing"
        ? (this.meal.preparing = this.preparationTime)
        : (this.meal.serving = this.servingTime);
        ...
        {this.meal && (
          <Fragment>
            ...
            <Typography variant="h6" align={"center"}>
              {this.meal.date.toLocaleDateString("en-EN", { weekday: "long" })}{" "}
              {this.meal.categoryType} - {this.meal.date.toLocaleDateString()}
            </Typography>
            <div className="mealDataDiv1">
              <TextField
                label="👨‍🍳Preparations start at"
                id="txtFldPreparing"
                type="time"
            ...
            <div className="mealDataDiv2">
              ...
              />
              <TextField
                label="Total food quantity limit"
                value={`${
                  (this.meal.totalDiners * this.meal.portion) / 1000
                } Kg`}
                variant="filled"
                className="txtFld noPointer"
                InputLabelProps={{ "aria-readonly": true }}
              ...
              <TextField
                ...
                InputProps={{
                  style: {
                    color:
                      this.mealQuantity <
                      (this.meal.portion * this.meal.totalDiners) / 1000
                        ? "green"
                        : "red",
                    backgroundColor:
                      this.mealQuantity <
              ...
              <TextField
                ...
                InputProps={{
                  style: {
                    color:
                      this.mealExpenses < this.meal.budget ? "green" : "red",
                    backgroundColor:
                      this.mealExpenses < this.meal.budget
                        ? "#F0FFFF"
                        : "#FFE4E1",
                  },
                }}
            ...
            <ExpansionPanel key={"use-info"} id="exPanel">
              ...
              <p>
                ★ Each dish can be added with only one ingredient of a kind, in
                case of split ingredient in a same dish,
              </p>
              <p>
                {" "}
                please enter the whole quantity you want and give a note for
                splitting the item.
              </p>
            </ExpansionPanel>
            <Box my={2}>
