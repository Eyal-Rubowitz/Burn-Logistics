...
class DinersNutritionComp extends PureComponent {
    ...
    render() {
        ...
        dietTypes = Object.keys(this.dinersNumber).map(dietType =>
            <div key={dietType} style={{ marginTop: 20, marginLeft: 20 }}>
                <Typography variant="h5" style={{ display: 'inline-block', marginTop: 12 }}>{dietType}: </Typography>
                <TextField label="Diners number"
                    ...
                    variant="outlined"
                    style={{ display: 'inline-block', width: '110px', marginLeft: 10, marginBottom: 20 }}
                    inputProps={{
                        type: "number"
                    }} />
                <IconButton onClick={() => { this.onDeleteDietType(dietType) }}
                    ...
                    style={{
                        marginLeft: 30,
                        visibility: ((Object.values(this.dinersNumber).length > 1 && this.isSetForAllMeals) || (Object.values(this.dinersNumber).length > 1 && this.selectedMealStore.meal)) ? 'visible' : 'hidden'
                    }}>
                ...
                <Paper style={{ display: 'block', width: '40%', margin: 'auto' }}>
                    ...
                    <div>
                        ...
                        <TextField label="Enter new diet type"
                            ...
                            variant="outlined"
                            style={{ marginLeft: 18, width: '35%', textAlign: 'center' }} />
                        <Fab
                            onClick={() => { this.onAddNewDietType() }}
                            variant='extended'
                            color='primary'
                            style={{ marginLeft: 20, height: 56, width: 90, display: 'inline-block', textAlign: 'center' }}>
                            <Icon>add</Icon>Add
                        </Fab>
                        <div style={{ marginTop: 30, marginLeft: 20, visibility: (this.isSetForAllMeals || this.selectedMealStore.meal) ? 'visible' : 'hidden' }}>
                            <div style={{ marginLeft: 20 }}>
                                <Typography variant="h5" style={{ marginTop: 76, marginBottom: 40, display: 'inline-block' }}>Total Diners: </Typography>
                                <TextField label=""
                                    value={this.totalDinersNum}
                                    variant="outlined"
                                    style={{ pointerEvents: 'none', marginLeft: 14, marginTop: 70, marginBottom: 40, width: '70px' }} />
                            </div>
                            {dietTypes}
                            <div>
                                <ExpansionPanel key={'info'} style={{ width: '98%', height: 'auto' }}>
                        ...
                        {/* <button style={{backgroundColor: (this.selectedMealStore.dateListLength === 0) ? 'green' : 'red' }}>Fooooo</button> */}
                    </div>
                </Paper>
            </Fragment>
        );
